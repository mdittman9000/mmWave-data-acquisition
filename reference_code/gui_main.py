...
class Window(QDialog):
    def __init__(self, parent=None, size=[]):
        ...
        self.setWindowTitle("mmWave Data Capture Tool")
    ...
    ('spectrum2', {'ticks': [(1.0, (255, 0, 0, 255)), (0.0, (255, 0, 255, 255))], 'mode': 'hsv'}),
        ...
        self.setRawDataRecordingLayout()
        ...
        # self.rangeProfileFig = plt.figure()
        # self.rangeCanvas = FigureCanvas(self.rangeProfileFig)
        # self.graphTabs.addTab(self.rangeCanvas, 'Range Profile')
        self.rawDataFeedLayout()
        self.graphTabs.addTab(self.rawDataFeed,'Raw Data Stream')

        # create video feed 
        self.videoThread = videoStreamThread(self, 0)
        self.videoFeed = VideoFrame(self.videoThread)
        self.videoTabs = QTabWidget()
        self.videoTabs.addTab(self.videoFeed, 'Video Stream')
        self.videoFeed.th.videoRecording.connect(self.manageRecording)

        gridlay = QGridLayout()
        gridlay.addWidget(self.comBox, 0,0,1,1)
        gridlay.addWidget(self.rawCollectBox, 1,0,1,1)
        gridlay.addWidget(self.statBox, 2,0,1,1)
        gridlay.addWidget(self.configBox,3,0,1,1)
        gridlay.addWidget(self.plotControlBox,4,0,1,1)
        gridlay.addWidget(self.recordBox,5,0,1,1)
        #gridlay.addWidget(self.boxTab,5,0,1,1)
        #gridlay.addWidget(self.spBox,6,0,1,1)
        gridlay.addWidget(self.graphTabs,0,1,5,1)
        gridlay.addWidget(self.videoTabs,5,1,4,1)
...
#
    def rawDataFeedLayout(self):
        self.rawDataFeed = QGroupBox('Raw Data Processor')
        self.rawDataFrameLabel = QLabel('Current Frame')
        self.rawDataFrameCount = QLabel('0')
        self.feedLayout = QGridLayout()
        self.feedLayout.addWidget(self.rawDataFrameLabel,0,0)
        self.feedLayout.addWidget(self.rawDataFrameCount,0,1)
        self.rawDataFeed.setLayout(self.feedLayout)
    ...
    def setConnectionLayout(self):
        ...
        self.configType.addItems(["SDK Out of Box Demo"])
    ...
    def setRawDataRecordingLayout(self):
        self.rawCollectBox = QGroupBox('Connect to DCA1000')
        self.dcaJSON = QLineEdit('') 
        self.dcaJSONSelect = QPushButton('Select DCA1000 Config')
        self.dcaJSONSelect.clicked.connect(self.selectDCAJSON)
        self.connectDCAButton = QPushButton('Connect DCA1000')
        self.connectDCAButton.clicked.connect(self.connectDCA)
        self.connectDCAStatus = QLabel('Disabled')
        self.skipProcessRawData = QCheckBox("Skip Raw Data Processing")
        self.rawDataLayout = QGridLayout()
        self.rawDataLayout.addWidget(self.dcaJSONSelect,0,0)
        self.rawDataLayout.addWidget(self.dcaJSON,0,1)
        self.rawDataLayout.addWidget(self.connectDCAButton,1,0)
        self.rawDataLayout.addWidget(self.connectDCAStatus,1,1)
        self.rawDataLayout.addWidget(self.skipProcessRawData,2,0)
        self.rawCollectBox.setLayout(self.rawDataLayout)

    def setRecordingLayout(self):
        self.recordBox = QGroupBox('Record Data')
        self.classType = QComboBox()
        self.classLabel = QLabel('Class Tag:')
        self.selectClassPath = QPushButton('Select Class List')
        self.selectClassPath.clicked.connect(self.selectClassFolder)
        self.classPath = QLineEdit("gestureList.txt") 
        self.postProcLabel = QLabel('Post Processing Type:')
        self.postProcType = QComboBox()
        self.postProcType.addItems(["Closest Point","Fastest Point"])
        self.selectRecordPath = QPushButton('Select Save Location')
        self.selectRecordPath.clicked.connect(self.selectRecordFolder)
        self.recordPath = QLineEdit('') 
        self.recordPath.setText("data")
        self.recordButton = QPushButton('Loading Video')
        self.recordButton.setEnabled(False)
        self.recordButton.clicked.connect(self.recordData)
        self.recordingStatus = QLabel('Idle')
        self.videoSrcButton = QPushButton('Change Video Source')
        self.videoSrcButton.clicked.connect(self.changeSource)
        self.videoSrc = QLineEdit('0') 
        self.instrucTagLabel = QLabel('While recording, press and hold \'F1\' to tag segment')
        self.instrucRecLabel = QLabel('To tag and record a short burst, press and hold \'F2\'')
        self.setClassSelector("gestureList.txt")
        self.recordLayout = QGridLayout()
        self.recordLayout.addWidget(self.postProcLabel,0,0)
        self.recordLayout.addWidget(self.postProcType,0,1)
        self.recordLayout.addWidget(self.classLabel,1,0)
        self.recordLayout.addWidget(self.classType,1,1)
        self.recordLayout.addWidget(self.selectClassPath,2,0)
        self.recordLayout.addWidget(self.classPath,2,1)
        self.recordLayout.addWidget(self.selectRecordPath,3,0)
        self.recordLayout.addWidget(self.recordPath,3,1)
        self.recordLayout.addWidget(self.recordButton,4,0)
        self.recordLayout.addWidget(self.recordingStatus,4,1)
        self.recordLayout.addWidget(self.videoSrcButton,5,0)
        self.recordLayout.addWidget(self.videoSrc,5,1)
        self.recordLayout.addWidget(self.instrucTagLabel,6,0,1,2)
        self.recordLayout.addWidget(self.instrucRecLabel,7,0,1,2)
        self.recordBox.setLayout(self.recordLayout)
    ...
    def saveData(self, parsedData):
        #Saves a recieved point cloud to CSV file
        pointCloud = parsedData[0]
        numPoints = parsedData[3]

        timeEllapsed = time.time() - self.startTime

        self.lastTimeRecorded = timeEllapsed

        #Write all avaible data to raw file for potential later use
        for i in range(numPoints):
            try:
                point = pointCloud[:,i]
            except:
                print("Lost Packet: Droping Frame")
                return

            line =  ",".join(str(v) for v in point)
            line = str(timeEllapsed) + ',' + str(i) + ',' + line
            self.rawOutputFile.write(line + '\n')
        
        if self.postProcType.currentText() == "None":

            if numPoints == 0:
                line = str(timeEllapsed) + ",0,0,0,0,0"
                self.outputFile.write(line + '\n')
                return

            for i in range(numPoints):
                point = pointCloud[:,i]
                line =  ",".join(str(v) for v in point[:-1])
                line = str(timeEllapsed) + ',' + str(i) + ',' + line
                self.outputFile.write(line + '\n')
        elif self.postProcType.currentText() == "Closest Point":
            if numPoints == 0:
                line = str(timeEllapsed) + ",0,0,0,0"
                self.outputFile.write(line + '\n')
                return
            ...
            for i in range(numPoints):
                point = pointCloud[:,i]
                dist.append(self.cartesianDistance(point[0],point[1],point[2]))

            minDist = dist.index(min(dist))

            point = pointCloud[:,minDist]
            line =  ",".join(str(v) for v in point[:-1])
            line = str(timeEllapsed) + ',' + line
            self.outputFile.write(line + '\n')
        elif self.postProcType.currentText() == "Fastest Point":
            if numPoints == 0:
                line = str(timeEllapsed) + ",0,0,0,0"
                self.outputFile.write(line + '\n')
                return
            ...
            maxVelo = velo.index(max(velo))

            point = pointCloud[:,maxVelo]
            line =  ",".join(str(v) for v in point[:-1])
            line = str(timeEllapsed) + ',' + line
            self.outputFile.write(line + '\n')
            

    def cartesianDistance(self, x, y, z):
    ...
    def updateGraph(self, parsedData):
        ...
        for i in range(numPoints):
            ...
            try:
                ...
                print("Packet Error: Lost a Packet, Resuming")
                return
        ...
        if (self.graphFin):
            ...
            if (self.threeD):
                ...
                # # 1D Ranger Profiler Plot
                # self.get_thread = updateQTTargetThread1D(rangeData, self.rangeProfileFig, self.rangeCanvas)
                # self.get_thread.done.connect(self.graphDone)
                # self.get_thread.start(priority=QThread.HighestPriority-1)
    ...
    def rawDataProcessed(self, data):
        #print(data)
        pass
    ...
    def keyUp(self, event):
        ...
        if not event.isAutoRepeat():
            if str(event.key()) == '16777264':
                if(self.recordingStatus.text() == 'Recording'):
                    self.recordingClass = False
                    self.videoFeed.classLabelCurrentlyRecording.setText("")

                    #if(self.classType.currentText() != "None"):
                    self.FrameRecordingTimeDuration = self.lastTimeRecorded - self.startFrameRecordingTime - 0.1
                    classType = self.classType.currentText().split('\t')[0].replace(" ","_")
                    label = ("{},{},{},{},{}\n").format(self.startFrameRecordingTime, self.FrameRecordingTimeDuration, classType, 1,"\"\"")
                    self.labelFile.write(label)

            #F2 Key Press
            if str(event.key()) == '16777265':
                if(self.recordingStatus.text() == 'Recording'):
                    self.videoFeed.classLabelCurrentlyRecording.setText("")
                    ...
                    # with open(self.fileName + "\\sampledData.csv",'r') as ifile:
                    #     header = ifile.readline()
                    #     startTime = float(ifile.readline().split(',')[0])

                    classType = self.classType.currentText().split('\t')[0].replace(" ","_")
                    label = ("{},{},{},{},{}\n").format(startTime, self.lastTimeRecorded, classType, 1,"\"\"")

                    try:
                        self.labelFile.write(label)
                    except:
                        className = self.classType.currentText().split('\t')[0].replace(" ","_")
                        self.labelFile = open(self.fileName + "\\" + className + ".label",'w')
                        self.labelFile.write(label)
    ...
    def keyPress(self, event):
        if not event.isAutoRepeat():
            if str(event.key()) == '16777264':
                if(self.recordingStatus.text() == 'Recording'):
                    self.startFrameRecordingTime = self.lastTimeRecorded - 0.1
                    self.videoFeed.classLabelCurrentlyRecording.setText(self.classType.currentText().split('\t')[0])

            if str(event.key()) == '16777265':
                if(self.recordingStatus.text() == 'Idle'):
                    self.recordData()
                    self.videoFeed.classLabelCurrentlyRecording.setText(self.classType.currentText().split('\t')[0])
                        
    def changeSource(self):
        self.videoTabs.removeTab(0)
        self.videoThread = videoStreamThread(self, int(self.videoSrc.text()))
        self.videoFeed = VideoFrame(self.videoThread)
        self.videoTabs.addTab(self.videoFeed, 'Video Stream')
    ...
    def recordData(self):
        if(self.recordingStatus.text() == 'Recording'):
            ...
            self.recordingStatus.setStyleSheet("color: red")
            self.recordButton.setText('Stop Recording')
            self.recordingClass = False

            #opening write file
            self.fileName = self.generateDataFileName(self.recordPath.text())
            self.outputFile = open(self.fileName + "\\sampledData.csv",'w')

            if self.postProcType.currentText() == "None":
                self.outputFile.write("Time (Sec), Obj Index, X (m), Y (m), Z (m), V(m/s)\n")
            else:
                self.outputFile.write("Time (Sec), X (m), Y (m), Z (m), V(m/s)\n")

            #opening raw output file
            self.rawOutputFile = open(self.fileName + "\\nonProcSampledData.csv",'w')
            self.rawOutputFile.write("Time (Sec), Obj Index, X (m), Y (m), Z (m), V(m/s), SNR\n")

            #writing to Imagimob Files
            self.generateImagimobFiles(self.fileName)
            ...
            try:
                self.uart_thread.fin.connect(self.saveData)
            except:
                print("No data is streaming")

            #starting video record
            self.videoFeed.startRecord(self.fileName + "\\sampledVideo.mp4")

    def guiStopRoutine(self):
        print("Stopping Data Recording Session")
        self.recordButton.setText('Record')
        self.recordingClass = False

        #Stopping recording thread
        try:
            self.uart_thread.fin.disconnect(self.saveData)
        except:
            pass
        ...
        self.recordingStatus.setStyleSheet("color: black")
        ...
        #update class count numbers
        self.setClassSelector(self.classPath.text(), keepIdx=self.classType.currentIndex())
        

    def manageRecording(self, recording):
        if not recording:
            ...
            self.recordButton.setText('Reloading Video')
            self.recordButton.setEnabled(False)
        else:
            self.recordButton.setText('Record')
    ...
    def generateImagimobFiles(self, baseDir):
        #Generates Labels and ImagimobSession for automatically tagging data
        fileName = self.classType.currentText().split('\t')[0].replace(" ","_")
        date = datetime.now().strftime("_%m_%d_%Y_%H_%M_%S")

        self.labelFile = open(baseDir + "\\" + fileName + ".label",'w')
        self.labelFile.write("Time(Seconds),Length(Seconds),Label(string),Confidence(double),Comment(string)\n")
        ...
        self.imsessionFile = open(baseDir + "\\" + fileName + date + ".imsession",'w')
        self.imsessionFile.write("<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n")
        self.imsessionFile.write("<ImagimobStudio>\n")
        self.imsessionFile.write(timeLineID)
        self.imsessionFile.write("    <Tracks>\n")
        self.imsessionFile.write("      <Track name=\"sampledVideo\" type=\"videomp4\">\n")
        self.imsessionFile.write("        <PayloadFile>sampledVideo.mp4</PayloadFile>\n")
        self.imsessionFile.write("      </Track>\n")
        self.imsessionFile.write("      <Track name=\"sampledData\" type=\"datacsv\">\n")
        if(self.connectDCAStatus.text() == "Connected"):
            self.imsessionFile.write("        <PayloadFile HasHeader=\"True\" DecimalPoint=\".\" FieldDelimiter=\",\">adcExtractedData.csv</PayloadFile>\n")
        else:
            self.imsessionFile.write("        <PayloadFile HasHeader=\"True\" DecimalPoint=\".\" FieldDelimiter=\",\">sampledData.csv</PayloadFile>\n")
        self.imsessionFile.write("      </Track>\n")
        self.imsessionFile.write("      <Track name=\"" + fileName +  "\" type=\"labelcsv\">\n")
        self.imsessionFile.write("        <PayloadFile HasHeader=\"True\" DecimalPoint=\".\" FieldDelimiter=\",\" DurationColumnIndex=\"1\" TextColumnIndex=\"2\" ConfidenceColumnIndex=\"3\" CommentColumnIndex=\"4\" TimeColumnIndex=\"0\" TimeUnit=\"Seconds\" DurationUnit=\"Seconds\">" + fileName + ".label</PayloadFile>\n")
        self.imsessionFile.write("      </Track>\n")
        self.imsessionFile.write("    </Tracks>\n")
        self.imsessionFile.write("  </Timeline>\n")
        self.imsessionFile.write("</ImagimobStudio>\n")
        self.imsessionFile.close()

    def generateDataFileName(self,baseDir):
        #Make Base dir if it doesn't exsit
        if not os.path.isdir(baseDir):
            os.mkdir(baseDir)

        #Make storage directory for current recording
        className = self.classType.currentText().split('\t')[0].replace(" ","_")
        dirName = baseDir + "\\" + className
        if not os.path.isdir(dirName):
            os.mkdir(dirName)

        date = datetime.now().strftime("_%m_%d_%Y_%H_%M_%S")
        
        fileName = dirName + "\\" +  className + date
        try:
            os.mkdir(fileName)
        except:
            pass
...
#TODO: create the disconnect button action
    ...
    def connectDCA(self):
        os.system("DCA1000EVM_CLI_Control.exe reset_ar_device " + self.dcaJSON.text())
        time.sleep(2)
        os.system("DCA1000EVM_CLI_Control.exe fpga " + self.dcaJSON.text())
        os.system("DCA1000EVM_CLI_Control.exe record " + self.dcaJSON.text())
        self.connectDCAStatus.setText('Connected')
        self.connectDCAStatus.setStyleSheet("color: green")

        #self.raw_thread = processRawDataFrameThread(self.rawDataSavePath + "\\adc_data_Raw_0.bin")
        #self.raw_thread.start(priority=QThread.HighestPriority-1)
    

    def selectRecordFolder(self):
        file = str(QFileDialog.getExistingDirectory(self,"Select Directory"))
        self.recordPath.setText(file)
        self.setClassSelector(self.classPath.text())

    def selectClassFolder(self):
        file = QFileDialog().getOpenFileName(filter="txt(*.txt)")[0]
        self.classPath.setText(file)
        self.setClassSelector(file)

    def selectDCAJSON(self):
        file = QFileDialog().getOpenFileName(filter="json(*.json)")[0]
        self.dcaJSON.setText(file)
        
        with open(self.dcaJSON.text()) as ifile:
            self.dcaJSONcontents = json.load(ifile)

        self.rawDataSavePath = self.dcaJSONcontents["DCA1000Config"]["captureConfig"]["fileBasePath"]


    def setClassSelector(self, file, keepIdx=-1):
        ...
        try:
            with open(file,'r') as clsFile:
                for line in clsFile:
                    self.classes.append(line.rstrip())
                    self.count.append(0)
                    self.timePerClass.append(0)
        except:
            for i in range(4):
                self.classes.append("Class " + str(i))
                self.count.append(0)
                self.timePerClass.append(0)

        #count the number of labels per data sample in the set save directory
        basedir = self.recordPath.text()
        for i in range(len(self.classes)):
            classdir = basedir + "\\" + self.classes[i].replace(' ', '_')
            if os.path.isdir(classdir):
                for filename in os.listdir(classdir):
                    sampledir = classdir + "\\" + filename + "\\" + self.classes[i].replace(' ', '_') + ".label"
                    try:
                        with open(sampledir, 'r') as ifile:
                            header = ifile.readline()
                            for line in ifile:
                                label = line.split(',')
                                self.timePerClass[i] = self.timePerClass[i] + float(label[1])
        ...
        for i in range(len(self.classes)):
            #formatedName = self.classes[i][:15] if len(self.classes[i]) > 15 else self.classes[i]
            self.classes[i] = self.classes[i] + '\t(' + str(self.count[i]) + ' samples)' + '\t(' + str(round(self.timePerClass[i],2)) + ' seconds)'

        self.classType.addItems(self.classes)

        if keepIdx != -1:
            self.classType.setCurrentIndex(keepIdx)
...
# TODO select the cfgfile automatically based on the profile.
    ...
    def selectFile(self):
        ...
        fd = QFileDialog()
        filt = "cfg(*.cfg)"
        filename = fd.getOpenFileName(filter=filt)    #deb_gp - added folder name
        return filename[0]
