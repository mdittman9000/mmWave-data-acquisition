...
# class updateQTTargetThread1D(QThread):
#     done = pyqtSignal()

#     def __init__(self, points, fig, canvas):
#         QThread.__init__(self)
#         self.points = points
#         self.fig = fig
#         self.canvas = canvas
...
class processRawDataFrameThread(QThread):
    fin = pyqtSignal('PyQt_PyObject')

    def __init__(self, recordingDir):
        super().__init__()
    ...
    def run(self):
        while(True):
            if self.processing:
                ...
                self.fin.emit(self.recordingDir)
                time.sleep(1)
                continue
            time.sleep(1)
...
class videoStreamThread(QThread):
    ...
    videoRecording = pyqtSignal(int, name='videoRecording')

    def __init__(self, test,videoSource):
        super().__init__()
            ...
            try:
                ...
                if ret:
                    ...
                    try:
                        ...
                        print("Video Stream Corrupted, Restarting")
                        self.videoRecording.emit(0)
                        self.out.release()
                        self.recording = False
                        self.run()
            except:
    ...
    def startRecord(self, fileName):
        ...
        self.out = cv2.VideoWriter(fileName,cv2.VideoWriter_fourcc(*'mp4v'), fps ,(640,480))
        self.recording = True
