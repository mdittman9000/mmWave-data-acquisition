...
class uartParserSDK():
    def __init__(self,type='(Legacy) 2D People Counting'):
        ...
        self.rangeProfile = np.zeros(self.maxPoints)
...
#It can parse the SDK3.x Compliant People Counting demo "tracker_dpc"
    ...
    def parseSDK3xRangeProfile(self, dataIn, dataSize):
        pointStruct = 'h'
        pointLength = struct.calcsize(pointStruct)

        self.rangeProfile = np.zeros(int(dataSize / 2))
        try:
            for i in range(int(dataSize / 2)):
                self.rangeProfile[i] = struct.unpack(pointStruct, dataIn[:pointLength])[0]
                dataIn = dataIn[pointLength:]
    ...
    def sdk3xTLVHeader(self, dataIn):
        ...
        while(1):
            ...
            if (magic != self.magicWord):
                ...
                #we have correct magic word, proceed to parse rest of data
                break
        #print("Total recieve length is " + str(len(dataIn)))
        dataIn = dataIn[headerLength:]
        remainingData = totalPacketLen - len(dataIn)
        #print("Total packet len" + str(totalPacketLen))
        count = 0
        ...
        #For case where bad parsing leads to a large number of TLVs and near infite looping
        if numTLVs > 10:
            return dataIn
        ...
        for i in range(numTLVs):
            ...
            if (tlvType == 1):
                ...
                self.parseSDK3xRangeProfile(dataIn[:tlvLength], tlvLength)
                dataIn = dataIn[tlvLength:]
            elif (tlvType == 7):
    ...
    def readAndParseUart(self):
        ...
        if (self.fail):
            return self.pcBufPing, self.targetBufPing, self.indexes, self.numDetectedObj, self.numDetectedTarget, self.frameNum, self.fail, self.classifierOutput, self.rangeProfile
        if (self.saveBinary):
        ...
        if (self.saveTextFile):
            if (self.frameNum%1000 == 0):
                ...
                fileName = 'data/pHistText_'+str(self.saveNumTxt)+'.csv'
                    ...
                    for i in range (1000):
                        ...
                        for j in range (np.count_nonzero(self.textStruct2D[i,1,:,0])): 
                            for k in range(4):
                                ...
                                tfile.write(',')
                            tfile.write('\n')
                        tfile.write('TID,x,y,vx,vy,ax,ay,Classification\n')
