...
class Window(QDialog):
    ...
    ('spectrum2', {'ticks': [(1.0, (255, 0, 0, 255)), (0.0, (255, 0, 255, 255))], 'mode': 'hsv'}),
        ...
        # create video feed 
        self.videoThread = videoStreamThread(0)
        ...
        gridlay.addWidget(self.recordBox,4,0,1,1)
        #gridlay.addWidget(self.boxTab,5,0,1,1)
        gridlay.addWidget(self.spBox,6,0,1,1)
        gridlay.addWidget(self.graphTabs,0,1,4,1)
        gridlay.addWidget(self.videoTabs,4,1,4,1)
...
#
    ...
    def setRecordingLayout(self):
        ...
        self.classType.addItems(["Left Swipe","Right Swipe","Up Swipe","Down Swipe","none"])
        self.selectClassPath = QPushButton('Select Class List')
        self.selectClassPath.clicked.connect(self.selectClassFolder)
        self.classPath = QLineEdit("classList.txt") 
        ...
        self.instrucTagLabel = QLabel('While recording, press and hold \'F1\' to tag gesture')
        ...
        self.recordLayout.addWidget(self.instrucTagLabel,5,0,1,2)
        self.recordLayout.addWidget(self.instrucRecLabel,6,0,1,2)
        ...
        if not event.isAutoRepeat():
            if str(event.key()) == '16777264':
                if(self.recordingStatus.text() == 'Recording'):
                    ...
                    self.FrameRecordingTimeDuration = self.lastTimeRecorded - self.startFrameRecordingTime
                    label = ("{},{},{},{},{}\n").format(self.startFrameRecordingTime, self.FrameRecordingTimeDuration, self.classType.currentText().replace(" ","_"), 1,"\"\"")
            ...
            if str(event.key()) == '16777265':
                if(self.recordingStatus.text() == 'Recording'):
                    ...
                    label = ("{},{},{},{},{}\n").format(startTime, self.lastTimeRecorded - startTime, self.classType.currentText().replace(" ","_"), 1,"\"\"")

                    try:
                        self.labelFile.write(label)
                    except:
                        className = self.classType.currentText().replace(" ","_")
    ...
    def keyPress(self, event):
        ...
        if not event.isAutoRepeat():
            if str(event.key()) == '16777264':
                if(self.recordingStatus.text() == 'Recording'):
                    self.startFrameRecordingTime = self.lastTimeRecorded
                    self.videoFeed.classLabelCurrentlyRecording.setText(self.classType.currentText())
    ...
    def generateImagimobFiles(self, baseDir):
        ...
        fileName = self.classType.currentText().replace(" ","_")
...
#
    ...
    def selectClassFolder(self):
        ...
        self.classType.addItems(classes)
